...
async def cmd_roulette(message: Message):
    ...
    parts = message.text.split()
    if len(parts) != 3:
        await message.answer("Использование: <code>/roulette 100 red</code> (red/black/green) "
                              "или <code>/roulette 100 17</code> (число 0-36)")
        return
    amount = parse_bet(parts[1], user["balance"])
    if amount is None:
        await message.answer("Некорректная ставка (проверь баланс и сумму).")
    ...
    if bet not in ("red", "black", "green"):
        if not (bet.isdigit() and 0 <= int(bet) <= 36):
            await message.answer("Ставка должна быть red/black/green или числом от 0 до 36.")
            return
    ...
    if win:
        ...
        await message.answer(f"🎡 Выпало: <b>{number}</b> ({color_ru})\n"
                              f"✅ Выигрыш x{multiplier}: <b>{winnings}</b> фишек!")
    else:
        db.change_balance(user["user_id"], -amount, won=False)
        await message.answer(f"🎡 Выпало: <b>{number}</b> ({color_ru})\n"
                              f"❌ Проигрыш <b>{amount}</b> фишек.")
...
async def cmd_rps(message: Message):
    ...
    if result == "win":
        db.change_balance(user["user_id"], amount, won=True)
        await message.answer(text + f"✅ Победа! Выигрыш: <b>{amount}</b> фишек")
    elif result == "lose":
        db.change_balance(user["user_id"], -amount, won=False)
        await message.answer(text + f"❌ Проигрыш <b>{amount}</b> фишек.")
    else:
...
async def cb_mine_click(callback: CallbackQuery):
    ...
    if len(state["revealed"]) == total_cells - state["mines_count"]:
        ...
        await bot.edit_message_text(
            chat_id=state["chat_id"], message_id=state["message_id"],
            text=f"🏆 <b>Все безопасные клетки открыты!</b>\nВыигрыш x{state['multiplier']}: <b>{winnings}</b> фишек 🪙",
        )
...
async def cmd_duel(message: Message):
    ...
    await message.answer(
        ...
        f"{message.from_user.first_name} вызывает {opponent.first_name} на ставку <b>{amount}</b> фишек!\n"
...
async def cb_duel_accept(callback: CallbackQuery):
    ...
    if challenger_balance < amount:
        await callback.message.edit_text("❌ У вызвавшего игрока больше не хватает фишек. Дуэль отменена.")
        await callback.answer()
        return
    if target_balance < amount:
        await callback.answer(f"У тебя не хватает фишек для ставки {amount}!", show_alert=True)
        return
